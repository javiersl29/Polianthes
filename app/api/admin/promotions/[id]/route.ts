...
export async function PATCH(req: NextRequest, { params }: { params: { id: string } }) {
  ...
  let i = 1;
  const allowed = [
    "title", "subtitle", "description", "type", "value", "bundle_price_cents", "required_size_ml", "mix_sizes", "mix_config",
    "quantity_to_take", "quantity_to_pay", "image_url", "image_prompt", "image_ai_generated",
    "badge_text", "badge_color", "min_items", "max_items", "min_subtotal_cents",
    "starts_at", "ends_at", "active", "sort_order"
  ];
  for (const k of allowed) {
    if (k in body) {
      let v: unknown = body[k];
      if (k === "mix_config" && v !== null && v !== undefined) {
